...
exports.importWarehouseShipments = exports.filterShipmentsByDatesAndStatus = exports.getShipmentSummary = exports.findDailyShipments = exports.shipmentStatusCounts = exports.searchShipmentByDate = exports.searchShipmentByStatus = exports.searchShipment = exports.deleteShipment = exports.getSingleShipment = exports.getAllShipments = exports.updateShipmentStatus = exports.updateShipment = exports.createShipment = void 0;
...
async function getNextShipmentNumber(countryCodeRaw) {
    const countryCode = String(countryCodeRaw || "").trim().toUpperCase() || "WH";
    const lastShipment = await shipment_model_1.default
...
function validateShipmentFields(body, type = "create") {
    const { user_id, reference, receiver_name, country, delivery_service, city, customer_address, national_address, location_coordinates, customer_email, contact_number, whatsApp_number, cod_amount, branded_items, custom_duty, goods_description, insurance, reforward_shipment, warehousing_location, } = body;
...
exports.createShipment = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        let lastShipment = await shipment_model_1.default
            .findOne({ shipment_no: { $regex: `^${countryCode}-\\d+$` } })
            .sort({ created_at: -1 });
        let nextNumber = 1;
        ...
        const newShipment = new shipment_model_1.default(data);
...
exports.updateShipment = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    try {
        const data = { ...req.body, updated_at: new Date() };
        const updatedShipment = await shipment_model_1.default.findByIdAndUpdate(id, data, { new: true, runValidators: true });
        if (!updatedShipment) {
...
exports.updateShipmentStatus = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const { status } = req.body;
    try {
        const updatedShipment = await shipment_model_1.default.findByIdAndUpdate(id, { status: status?.toLowerCase() }, { new: true, runValidators: true });
        if (!updatedShipment) {
            return res.status(404).json({
                code: 404,
                status: false,
...
exports.getAllShipments = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        const [shipments, total] = await Promise.all([
            shipment_model_1.default.find(query).sort({ created_at: -1 }).skip(skip).limit(limitNum).lean(),
            shipment_model_1.default.countDocuments(query),
        ]);
...
exports.getSingleShipment = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    try {
        const shipment = await shipment_model_1.default.findById(id);
        if (!shipment) {
            return res.status(404).json({
                code: 404,
                status: false,
...
exports.deleteShipment = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    try {
        const deletedShipment = await shipment_model_1.default.findByIdAndDelete(id);
        if (!deletedShipment) {
            return res.status(404).json({
                code: 404,
                status: false,
...
exports.searchShipment = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { reference_number, receiver_name, contact_number, shipment_no, user_type, user_id, } = req.body;
    try {
        const query = {};
        ...
        const shipments = await shipment_model_1.default.find(query).lean();
        if (!shipments.length) {
            return res.status(404).json({
                code: 404,
                status: false,
...
exports.searchShipmentByStatus = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        const shipments = await shipment_model_1.default.find(query)
            .sort({ created_at: -1 })
            .limit(5);
...
exports.searchShipmentByDate = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
...
exports.shipmentStatusCounts = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const statusList = [
        "delivered",
        "in_transit",
        "no_answer",
    ...
    if (user_type === "admin") {
        ...
        matchQuery.user_id = new mongoose_1.default.Types.ObjectId(user_id);
    }
    else {
        return res.status(403).json({
            code: 403,
    ...
    try {
        ...
        const statusCounts = await shipment_model_1.default.aggregate(statusPipeline);
        const statusMap = {};
        ...
        const cityWiseCounts = await shipment_model_1.default.aggregate(cityPipeline);
        /* =========================
          COUNTRY-WISE COUNTS
       ========================= */
        ...
        const countryWiseCounts = await shipment_model_1.default.aggregate(countryPipeline);
        /* =========================
           DATE RANGES
        ========================= */
        const now = new Date();
        ...
        /* =========================
                QUERY BUILDER
        ========================= */
        const buildDateQuery = (startDate) => ({
            ...matchQuery,
        ...
        /* =========================
                COUNTS
        ========================= */
        const [shippedToday, shippedThisWeek, shippedThisMonth, shippedThisYear,] = await Promise.all([
            shipment_model_1.default.countDocuments(buildDateQuery(startOfToday)),
            shipment_model_1.default.countDocuments(buildDateQuery(startOfWeek)),
            shipment_model_1.default.countDocuments(buildDateQuery(startOfMonth)),
            shipment_model_1.default.countDocuments(buildDateQuery(startOfYear)),
        ]);
        /* =========================
           TOTAL SHIPMENTS
        ========================= */
        const totalShipments = Object.keys(matchQuery).length
            ? await shipment_model_1.default.countDocuments(matchQuery)
            : await shipment_model_1.default.estimatedDocumentCount();
        res.status(200).json({
            code: 200,
            status: true,
...
exports.findDailyShipments = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        const dailyShipments = await shipment_model_1.default.find(query);
        res.status(200).json({
            code: 200,
            status: true,
...
exports.getShipmentSummary = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        if (user_type === "admin") {
            if (user_id && mongoose_1.Types.ObjectId.isValid(user_id)) {
                query.user_id = new mongoose_1.Types.ObjectId(user_id);
            }
        }
        else if (user_type === "user") {
            if (!user_id || !mongoose_1.default.Types.ObjectId.isValid(user_id)) {
        ...
        if (hasCountryFilter) {
            ...
            const countryResult = await shipment_model_1.default.aggregate(countryWisePipeline);
            const countryWiseSummary = countryResult.length > 0
                ? countryResult[0]
            ...
            const statusCounts = await shipment_model_1.default.aggregate(statusWisePipeline);
            const statusWiseSummary = {};
            let totalCount = 0;
...
exports.filterShipmentsByDatesAndStatus = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        const { from_date, to_date, updated_at, status, user_type, user_id } = req.body;
        let query = {};
        if (user_type === "admin") {
            // admins can see all shipments
        }
        else if (user_type === "user" && user_id && mongoose_1.Types.ObjectId.isValid(user_id)) {
            query.user_id = new mongoose_1.Types.ObjectId(user_id);
        }
        else {
            return res.status(403).json({
        ...
        const shipments = await shipment_model_1.default.find(query).sort({ created_at: -1 });
        return res.status(200).json({
            code: 200,
            status: true,
            message: "Shipments fetched successfully",
...
exports.importWarehouseShipments = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    try {
        ...
        for (let index = 0; index < rows.length; index += 1) {
            ...
            if (!mongoose_1.Types.ObjectId.isValid(resolvedUserId)) {
                errors.push({
                    row: rowNumber,
            ...
            const countryId = row.country_id && row.country_id.trim() !== ""
                ? mongoose_1.Types.ObjectId.isValid(row.country_id)
                    ? new mongoose_1.Types.ObjectId(row.country_id)
                    : null
                : undefined;
            const deliveryServiceId = row.delivery_service_id && row.delivery_service_id.trim() !== ""
                ? mongoose_1.Types.ObjectId.isValid(row.delivery_service_id)
                    ? new mongoose_1.Types.ObjectId(row.delivery_service_id)
                    : null
                : undefined;
            const cityId = row.city_id && row.city_id.trim() !== ""
                ? mongoose_1.Types.ObjectId.isValid(row.city_id)
                    ? new mongoose_1.Types.ObjectId(row.city_id)
                    : null
                : undefined;
            ...
            const shipmentNo = row.shipment_no && row.shipment_no.trim() !== ""
            ...
            preparedDocs.push({
                ...
                doc: {
                    ...
                    user_id: new mongoose_1.Types.ObjectId(resolvedUserId),
        ...
        let importedCount = 0;
        const saveResults = await Promise.allSettled(preparedDocs.map((item) => shipment_model_1.default.create(item.doc)));
